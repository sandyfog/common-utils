...
public class RegexpStressTest {
	...
	public static List<Object[]> data() {
		...
		String[] data = new String[] {
				"control.setTemplate(aaa)",
				"screen_placeholder",
				"stringEscapeUtil.escape($abc)",
				"csrfToken.getHiddenField()",
				"sdfasdfasldkfjqwpoieruqpweiorjalkfjasd;klfjasdpoifuqwpioerjkjdsfsladfjpqoieadsfkasdfdaksfj;asdkfa;sdwr" };

		add(list, new String[] { "^control\\.", "^screen_placeholder",
				"^stringEscapeUtil\\.escape", "^csrfToken\\.(get)?hiddenField",
	...
	private static void add(List<Object[]> list, String[] exps, String[] data,
			int matchedCount) {
	...
	private void test(final Tester test, final int loop, final int concurrency)
			throws Exception {
		test.warmUp();
		...
		System.out.printf("%s - %,d ms for %,d matches%n", test, duration,
				matches);
	...
	class OroTester implements Tester {
		...
		public OroTester() throws Exception {
			...
			for (int i = 0; i < patterns.length; i++) {
				patterns[i] = new Perl5Compiler().compile(exps[i],
	...
	class JdkRegexTester implements Tester {
		...
		public JdkRegexTester() throws Exception {
			...
			for (int i = 0; i < patterns.length; i++) {
				patterns[i] = java.util.regex.Pattern.compile(exps[i],
