...
public class DivisionManagerTest {
	...
	public void getDivisionById() {
		assertEquals(divisionManager.getDivisionById(120106).getDivisionName(),
				"红桥区");
	}

	@Test
	public void getDivisionByName() {
		assertEquals(divisionManager.getDivisionByName("红桥区").get(0)
				.getDivisionName(), "红桥区");
	}

	@Test
	public void getDivisionByAbbName() {
		assertEquals(divisionManager.getDivisionByAbbName("内蒙古").get(0)
				.getDivisionName(), "内蒙古自治区");
	}

	@Test
	public void getDivisionByTName() {
		assertEquals(
				((ChinaDivision) divisionManager.getDivisionByTName("澳門特別行政區")
						.get(0)).getDivisionTname(), "澳門特別行政區");
	}

	@Test
	public void getDivisionByZip() {
		assertEquals(divisionManager.getDivisionByZip("300131").get(0)
				.getDivisionName(), "红桥区");
	}
	...
	public void getObscureDivisionByTName() {
		for (int i = 0; i < LOOPS; i++) {
			assertEquals(
					divisionManager.getObscureDivisionByTName("澳门").size(), 2);
		}
	...
	public void testPerformance() throws InterruptedException {
		...
		System.out.println("总失败次数: " + failCount + " of "
				+ (LOOPS * THREAD_COUNT));
	}
