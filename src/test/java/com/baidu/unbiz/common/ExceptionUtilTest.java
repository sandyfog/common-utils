...
public class ExceptionUtilTest {
	...
	public void causedBy() {
		...
		assertTrue(ExceptionUtil.causedBy(getException(),
		...
		assertFalse(ExceptionUtil.causedBy(getException(),
	...
	public void getRootCause() {
		...
		// 3 exceptions
		assertThat(ExceptionUtil.getRootCause(getException()),
	...
	public void toRuntimeException() {
		...
		assertEquals(
				RuntimeException.class,
				ExceptionUtil.toRuntimeException(new Exception(),
						PrivateRuntimeException.class).getClass());
	...
	public void toRuntimeException2() {
		...
		assertSame(iae, ExceptionUtil.toRuntimeException(iae,
				IllegalStateException.class));
		assertTrue(ExceptionUtil.toRuntimeException(ioe,
				IllegalStateException.class) instanceof IllegalStateException);
		assertSame(
				ioe,
				ExceptionUtil.toRuntimeException(ioe,
						IllegalStateException.class).getCause());
	}
	...
	public void throwRuntimeExceptionOrError() {
		...
		try {
			ExceptionUtil.throwRuntimeExceptionOrError(new RuntimeException(
					"test"));
			fail();
		} catch (RuntimeException e) {
			assertEquals("test", e.getMessage());
	...
	public void getStackTrace() {
		...
		assertTrue(stacktrace.contains(ExceptionUtilTest.class.getName()
				+ ".getStackTrace"));
	}
