...
public class StringEscapeUtilTest {
	...
	public void escapeURLStrict() throws Exception {
		...
		// 测试中文
		assertEquals("%D6%D0%BB%AA%C8%CB%C3%F1%B9%B2%BA%CD%B9%FA",
				StringEscapeUtil.escapeURL("中华人民共和国", "GBK"));
	...
	public void escapeURLLoose() throws Exception {
		...
		assertEquals("%3B%2F%3F%3A%40%26%3D%2B%24%2C",
				StringEscapeUtil.escapeURL(reserved, "8859_1", false));
		...
		assertEquals(expectedStr,
				StringEscapeUtil.escapeURL(str, "8859_1", false));
		...
		str = "中华人民共和国\u3000";
		assertEquals("中华人民共和国%A1%A1",
				StringEscapeUtil.escapeURL(str, "GBK", false));
	...
	public void escapeURLEncoding() {
		...
		assertEquals("%D6%D0%BB%AA%C8%CB%C3%F1%B9%B2%BA%CD%B9%FA",
				StringEscapeUtil.escapeURL("中华人民共和国"));

		LocaleUtil.setContext(Locale.US, "8859_1");
		assertEquals("%3F%3F%3F%3F%3F%3F%3F",
				StringEscapeUtil.escapeURL("中华人民共和国"));
	...
	public void unescapeURLEncoding() {
		...
		assertEquals(
				"中华人民共和国",
				StringEscapeUtil
						.unescapeURL("%D6%D0%BB%AA%C8%CB%C3%F1%B9%B2%BA%CD%B9%FA"));
	...
	public void unescapeURL_rawBytes() throws Exception {
		...
		assertEquals("Justin\u00b7Bieber",
				StringEscapeUtil.unescapeURL(escaped));
		assertEquals("中华民国100年", StringEscapeUtil.unescapeURL(
				reencode("中华民国100年", "GBK"), "GBK"));

		// 混合中/英文、编码/未编码
		assertEquals("中华abc 人民共和国abc 人", StringEscapeUtil.unescapeURL(
				"中华abc+\310%CB民共和国abc+\310%CB", "GBK"));
		...
		assertEquals("abc中华", StringEscapeUtil.unescapeURL(
				reencode("abc中", "GBK") + "华", "GBK"));
		assertEquals("中华abc", StringEscapeUtil.unescapeURL(reencode("中", "GBK")
				+ "华abc", "GBK"));
