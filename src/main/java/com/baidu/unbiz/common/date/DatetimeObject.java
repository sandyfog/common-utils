...
public class DatetimeObject implements Comparable<Object>, Cloneable {
	...
	public static final JulianDateStamp JD_1970 = new JulianDateStamp(2440587,
			0.5);

	public static final JulianDateStamp JD_2001 = new JulianDateStamp(2451910,
			0.5);
	...
	public void setDateTimeStamp(DateTimeStamp dts) {
		set(dts.year, dts.month, dts.day, dts.hour, dts.minute, dts.second,
				dts.millisecond);
	}
	...
	private void setJdOnly(int year, int month, int day, int hour, int minute,
			int second, int millisecond) {
	...
	private static final int NUM_DAYS[] = { -1, 0, 31, 59, 90, 120, 151, 181,
			212, 243, 273, 304, 334 }; // 1-based
	private static final int LEAP_NUM_DAYS[] = { -1, 0, 31, 60, 91, 121, 152,
			182, 213, 244, 274, 305, 335 }; // 1-based
	...
	private int calcWeekOfYear(int start, int must) {
		...
		if (DayOfYearNumber < 1) {
			time_year--;
			DayOfYearNumber = TimeUtil.isLeapYear(time_year) ? 366 + DayOfYearNumber
					: 365 + DayOfYearNumber;
		...
		if ((DayOfYearNumber <= (8 - Jan1WeekDay)) && (Jan1WeekDay > must)) {
			YearNumber--;
			if ((Jan1WeekDay == must + 1)
					|| ((Jan1WeekDay == must + 2) && (TimeUtil
	...
	public void add(int year, int month, int day, int hour, int minute,
			int second, int millisecond, boolean monthFix) {
		int difference = 0;
		if (trackDST) {
	...
	protected void addNoDST(int year, int month, int day, int hour, int minute,
			int second, int millisecond, boolean monthFix) {
		...
		if (monthFix == false) {
			...
			setJdOnly(time.year, month, time.day, time.hour, time.minute,
					time.second, time.millisecond);
			if (time.day < from) {
				set(time.year, time.month, 0, time.hour, time.minute,
						time.second, time.millisecond);
			} else {
				calculateAdditionalData();
			}
	...
	public void sub(int year, int month, int day, int hour, int minute,
			int second, int millisecond, boolean monthFix) {
		add(-year, -month, -day, -hour, -minute, -second, -millisecond,
				monthFix);
	}
	...
	public DatetimeObject(int year, int month, int day, int hour, int minute,
			int second, int millisecond) {
	...
	public void setTimeInMillis(long millis) {
		...
		double fraction = (double) (millis % TimeUtil.MILLIS_IN_DAY)
				/ TimeUtil.MILLIS_IN_DAY;
	...
	public long getTimeInMillis() {
		double then = (jdate.fraction - JD_1970.fraction)
				* TimeUtil.MILLIS_IN_DAY;
	...
	private static final int MONTH_LENGTH[] = { 0, 31, 0, 31, 30, 31, 30, 31,
			31, 30, 31, 30, 31 };
	...
	protected TimeZone timezone = DefaultDatetime.timeZone == null ? TimeZone
			.getDefault() : DefaultDatetime.timeZone;

	public void changeTimeZone(TimeZone timezone) {
		long now = getTimeInMillis();
		int difference = TimeZoneUtil.getOffsetDifference(now, this.timezone,
				timezone);
	...
	protected Locale locale = DefaultDatetime.locale == null ? Locale
			.getDefault() : DefaultDatetime.locale;
