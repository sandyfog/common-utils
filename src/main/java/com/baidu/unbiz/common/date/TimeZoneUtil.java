...
public abstract class TimeZoneUtil {
	...
	public static int getOffset(DatetimeObject jdt, TimeZone tz) {
		return tz.getOffset(jdt.getEra(), jdt.getYear(), jdt.getMonth() - 1,
				jdt.getDay(), TimeUtil.toCalendarDayOfWeek(jdt.getDayOfWeek()),
				jdt.getMillisOfDay());
	}

	public static int getOffsetDifference(DatetimeObject jdt, TimeZone from,
			TimeZone to) {
