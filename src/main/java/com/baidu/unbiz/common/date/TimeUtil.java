...
public abstract class TimeUtil {
	...
	public static int dayOfYear(int year, int month, int day) {
		...
		if (isLeapYear(year)) {
			...
			day_of_year = ((275 * month) / 9) - (((month + 9) / 12) << 1) + day
					- 30;
		}
		return day_of_year;
	}
	...
	private static final int[] MONTH_LENGTH = { 0, 31, 28, 31, 30, 31, 30, 31,
			31, 30, 31, 30, 31 };
	private static final int[] LEAP_MONTH_LENGTH = { 0, 31, 29, 31, 30, 31, 30,
			31, 31, 30, 31, 30, 31 };
	...
	public static boolean isValidDateTime(DateTimeStamp dts) {
		return (isValidDate(dts.year, dts.month, dts.day) && isValidTime(
				dts.hour, dts.minute, dts.second, dts.millisecond));
	}

	public static JulianDateStamp toJulianDate(DateTimeStamp time) {
		return toJulianDate(time.year, time.month, time.day, time.hour,
				time.minute, time.second, time.millisecond);
	}

	public static JulianDateStamp toJulianDate(int year, int month, int day,
			int hour, int minute, int second, int millisecond) {
		...
		int jd;
		if (year <= 0) {
			jd = (int) ((365.25 * iy0) - 0.75) + (int) (30.6001 * (im0 + 1))
					+ day + 1720994;
		} else {
			jd = (int) (365.25 * iy0) + (int) (30.6001 * (im0 + 1)) + day
					+ 1720994;
		}
