...
public abstract class FileUtil {
	...
	public static File toFile(URL url) {
		...
		return (path != null) ? new File(StringEscapeUtil.unescapeURL(path))
				: null;
	...
	public static File[] listDirSuffixFiles(File dir, final String suffix) {
		...
		return dir.listFiles(new FileFilter() {
			public boolean accept(File file) {
				return StringUtil.isEmpty(suffix) ? true : (file.getName()
						.endsWith(suffix));
			}
		});
	}
	 ...
	 */
	public static File[] listDirAllConditionFiles(File dir,
	 ...
	 */
	public static File[] listDirAllConditionFiles(String dirPath,
			final boolean... conditions) {
		if (!exist(dirPath)) {
			return null;
		}
	 ...
	 */
	public static File[] listDirAnyConditionFiles(File dir,
			final boolean... conditions) {
		if (dir == null) {
			return null;
		}
	 ...
	 */
	public static File[] listDirAnyConditionFiles(String dirPath,
			final boolean... conditions) {
		if (!exist(dirPath)) {
			return null;
		}
	 ...
	 *             如果路径非法
	 */
	public static String normalizePath(String path, boolean removeTrailingSlash)
			throws IllegalPathException {
	 ...
	 *             如果路径非法
	 */
	public static String getAbsolutePathBasedOn(String basedir, String path)
			throws IllegalPathException {
		// 如果path为绝对路径，则规格化后返回
	 ...
	 *             如果basedir不是绝对路径
	 */
	public static String getSystemDependentAbsolutePathBasedOn(String basedir,
			String path) {
		path = StringUtil.trimToEmpty(path);
		...
		if (pathFile.isAbsolute()) {
			...
			if (baseFile.isAbsolute()) {
				...
				throw new IllegalPathException("Basedir is not absolute path: "
						+ basedir);
			}
		}
	...
	public static FileNameAndExtension getFileNameAndExtension(String path,
		...
		return new FileNameAndExtension(fileName, extension,
	...
	public static class FileNameAndExtension {
		...
		private FileNameAndExtension(String fileName, String extension,
				boolean extensionToLowerCase) {
			this.fileName = fileName;
			this.extension = extensionToLowerCase ? StringUtil
					.toLowerCase(extension) : extension;
		}
	...
	public static String resolve(String url, String relativePath) {
		...
		return url.substring(0, index)
				+ normalizeAbsolutePath(url.substring(index) + "/../"
						+ relativePath);
	}
	...
	public static File createTempFile(String prefix, String suffix, File tempDir)
			throws IOException {
		return createTempFile(prefix, suffix, tempDir, true);
	}

	public static File createTempFile(String prefix, String suffix,
			String tempDirName, boolean create) throws IOException {
		return createTempFile(prefix, suffix, new File(tempDirName), create);
	}

	public static File createTempFile(String prefix, String suffix,
			File tempDir, boolean create) throws IOException {
		// FIXME
	...
	private static File doCreateTempFile(String prefix, String suffix, File dir)
			throws IOException {
