 ...
 * 一旦创建就不可更改，对BigDecimal进行任意运算都会生成一个新的 BigDecimal对象，因此对于大批量统计的性能不够满意。Money类支持
 * mutable，对大批量统计提供较好的支持。
 * <p>
 * mutable的方法为：{@link #addTo} 、{@link #subtractFrom} 、{@link #multiplyBy} 等。
 * {@link #divideBy}
 * <li>提供基本的格式化功能。
 * <li>Money类中不包含与业务相关的统计功能和格式化功能。
 * <li>Money类实现了Serializable接口，支持作为远程调用的参数和返回值。
 * <li>Money类实现了equals和hashCode方法。
 * </ul>
 * 
 * @author <a href="mailto:xuchen06@baidu.com">xuc</a>
 * @version create on 2014年7月24日 上午12:30:25
 */
public class Money implements Serializable, Comparable<Object> {
	...
	public BigDecimal getAmount() {
		return BigDecimal.valueOf(this.cent,
	...
	public void setAmount(BigDecimal amount) {
		if (amount != null) {
			this.cent = rounding(amount.movePointRight(2),
	...
	public Money divide(BigDecimal val, int roundingMode) {
		BigDecimal newCent = BigDecimal.valueOf(this.cent).divide(val,
				roundingMode);
	...
	public Money[] allocate(long[] ratios) {
		...
		for (int i = 0; i < results.length; i++) {
			results[i] = newMoneyWithSameCurrency((this.cent * ratios[i])
					/ total);
	...
	public String getDisplayUnit(Locale e) {
		...
		Map<?, ?> cMap = (Map<?, ?>) CURRENCY_DISPLAY_UNIT_MAP
				.get(e.toString());
		if (cMap == null) {
