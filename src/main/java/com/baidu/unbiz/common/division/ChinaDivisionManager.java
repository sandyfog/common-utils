 ...
 */
public class ChinaDivisionManager extends CachedLogger implements
		DivisionManager {

	private static final Logger logger = LoggerFactory
			.getLogger(ChinaDivisionManager.class);
	...
	/** 部定义编号对应的对象 */
	private final Map<Integer, ChinaDivision> idMap = CollectionUtil
			.createHashMap();
	/** 国标缩写对应的对象 */
	private final Map<String, List<Division>> abbNameMap = CollectionUtil
			.createHashMap();
	/** 名字对应的对象 */
	private final Map<String, List<Division>> nameMap = CollectionUtil
			.createHashMap();
	/** 邮编对应的对象 */
	private final Map<String, List<Division>> zipMap = CollectionUtil
			.createHashMap();
	...
	private List<String> loadRowData() {
		...
		try {
			return ReaderUtil.readLinesAndClose(this.getClass()
					.getResourceAsStream(divisionData));
		} catch (IOException e) {
	...
	private ChinaDivision create(String[] strings) throws Exception {
		...
		chinaDivision.setDivisionTname(GBKMap.covGBKS2T(chinaDivision
				.getDivisionName()));
	...
	private void transformData(List<String> lines) {
		for (String line : lines) {
			...
			try {
				...
				logger.error("transformData error ", e, strings[0], strings[1],
						strings[2]);
				continue;
			}
	...
	private synchronized void initDivision() throws DivisionException {
		if (!init) {
			...
			logger.infoIfEnabled(
					"load china division finished. have {}records, spend {}ms.",
	...
	private void initIdMap() {
		for (Map.Entry<Integer, ChinaDivision> entry : idMap.entrySet()) {
			...
			if (chinaDivision != null
				...
				ChinaDivision parent = idMap.get(chinaDivision
						.getParentDivision().getDivisionId());
				if (parent == null) {
					continue;
				}
	...
	private void initNameMap() {
		for (Map.Entry<Integer, ChinaDivision> entry : idMap.entrySet()) {
			...
			List<Division> divisionList = nameMap.get(chinaDivision
					.getDivisionName());
	...
	private void initAbbNameMap() {
		for (Map.Entry<Integer, ChinaDivision> entry : idMap.entrySet()) {
			...
			List<Division> divisionList = abbNameMap.get(chinaDivision
					.getDivisionAbbName());
			if (divisionList == null) {
				divisionList = CollectionUtil.createArrayList();
			}
	...
	private void initZipMap() {
		for (Map.Entry<Integer, ChinaDivision> entry : idMap.entrySet()) {
			...
			List<Division> divisionList = zipMap.get(chinaDivision
					.getDivisionZip());
			if (divisionList == null) {
				divisionList = CollectionUtil.createArrayList();
			}
	...
	public List<Division> getProvinceDisivion() {
		ChinaDivision chinaDivision = this
				.getDivisionById(ChinaDivision.DEFAULT_ALL_DIVISION);
	...
	public boolean isCityDivision(int id) {
		...
		if ((chinaDivision != null)
				...
				&& (chinaDivision.getParentDivision().getParentDivision()
						.getDivisionId() == ChinaDivision.DEFAULT_ALL_DIVISION)) {
			return true;
		}
	...
	public boolean isRegionDivision(int id) {
		...
		if ((chinaDivision != null)
				...
				&& (chinaDivision.getParentDivision().getParentDivision() != null)
				&& (chinaDivision.getParentDivision().getParentDivision()
						.getParentDivision() != null)
				&& (chinaDivision.getParentDivision().getParentDivision()
						.getParentDivision().getDivisionId() == ChinaDivision.DEFAULT_ALL_DIVISION)) {
			return true;
		}
