...
public abstract class TestUtil {
	...
	public static <T extends Throwable> Matcher<T> exception(
			Class<? extends Throwable> cause, String... snippets) {
	...
	public static File getClassesDir(Class<?> classWithinDir) {
		return getClassesDir(classWithinDir == null ? null : classWithinDir
				.getName());
	}

	public static File getClassesDir(String classWithinDir) {
		String clazzResourceName = getResourceNameOfClass(classWithinDir)
				+ ".class";
		File classFile;

		try {
			classFile = new File(Thread.currentThread().getContextClassLoader()
					.getResource(clazzResourceName).toURI());
		} catch (URISyntaxException e) {
			throw new RuntimeException("Could not find classes dir of "
					+ classWithinDir, e);
		}

		File classesdir = classFile.getParentFile();

		for (; classesdir != null && classesdir.isDirectory(); classesdir = classesdir
				.getParentFile()) {
	...
	public static Field getAccessibleField(Class<?> targetType, String fieldName) {
		...
		for (Class<?> c = targetType; c != null && field == null; c = c
				.getSuperclass()) {
			try {
		...
		assertNotNull("field " + fieldName + " not found in " + targetType,
				field);
	...
	/** 取得method，并设置为可访问。 */
	public static Method getAccessibleMethod(Class<?> targetType,
			String methodName, Class<?>[] argTypes) {
		assertNotNull("missing targetType", targetType);

		Method method = null;

		for (Class<?> c = targetType; c != null && method == null; c = c
				.getSuperclass()) {
			try {
		...
		assertNotNull("method " + methodName + " not found in " + targetType,
				method);
	...
	public static <T> T getFieldValue(Object target, Class<?> targetType,
	...
	public static <T> T invokeMethod(Object target, String methodName,
			Class<?>[] argTypes, Object[] args, Class<T> returnType)
			throws IllegalArgumentException, IllegalAccessException,
			InvocationTargetException {
		return invokeMethod(target, null, methodName, argTypes, args,
				returnType);
	}

	/** 执行方法，即使private也没关系。 */
	@SuppressWarnings("unchecked")
	public static <T> T invokeMethod(Object target, Class<?> targetType,
			String methodName, Class<?>[] argTypes, Object[] args,
			Class<T> returnType) throws IllegalArgumentException,
