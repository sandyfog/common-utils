...
public class CsvProcessor implements FileProcessor {
	...
	public void importBean(Object bean, String filePath) throws IOException {
		...
		WriterUtil.writeLinesAndClose(filePath, false, GBK, new Object[] {
				title, data });
	}

	@Override
	public void importBeans(Class<?> clazz, Object[] beans, String filePath)
			throws IOException {
		List<FieldDesc> list = processeCache.getFieldDescs(clazz);
		Assert.assertTrue(CollectionUtil.isNotEmpty(list),
				"FieldDesc is not empty");
	...
	@Override
	public void importBeans(String[] contents, String filePath)
			throws IOException {
	...
	@Override
	public void importBeans(List<String> contents, String filePath)
			throws IOException {
	...
	public <T> T exportBean(Class<T> clazz, String filePath) throws IOException {
		...
		List<String> lines = ReaderUtil.readLinesAndClose(filePath,
				Charset.forName(GBK));
		List<String> titles = processeCache.getTitles(clazz);
		int size = lines.size();
	...
	@Override
	public <T> List<T> exportBeans(Class<T> clazz, String filePath)
			throws IOException {
		List<FieldDesc> list = processeCache.getFieldDescs(clazz);
		Assert.assertTrue(CollectionUtil.isNotEmpty(list),
				"FieldDesc is not empty");
	...
	private <T> List<T> linesToList(Class<T> clazz, List<FieldDesc> list,
			List<String> lines) {
	...
	private <T> T stringsToBean(Class<T> clazz, List<FieldDesc> list,
			String[] strings) {
		...
		for (int i = 0, size = strings.length; i < size; i++) {
			...
			if (value != null) {
				...
				ReflectionUtil.writeField(field, bean,
	...
	@Override
	public void importBeans(String[] headers, Class<?> clazz, Object[] beans,
			String filePath) throws IOException {
		List<FieldDesc> list = processeCache.getFieldDescs(clazz);
		Assert.assertTrue(CollectionUtil.isNotEmpty(list),
				"FieldDesc is not empty");

		List<String> datas = CollectionUtil.createArrayList(headers.length
				+ beans.length);
	...
	@Override
	public void importBeans(ListMap<String, String> headers, Class<?> clazz,
			Object[] beans, String filePath) throws IOException {
		List<FieldDesc> list = processeCache.getFieldDescs(clazz);
		Assert.assertTrue(CollectionUtil.isNotEmpty(list),
				"FieldDesc is not empty");

		List<String> datas = CollectionUtil.createArrayList(headers.size()
				+ beans.length);

		for (int index = 0, size = headers.size(); index < size; index++) {
			datas.add(CsvUtil.toCsvString(headers.getKey(index),
					headers.get(index)));
		}
	...
	public List<String> collectData(FileBeanInfo beanInfo) throws IOException {
		...
		int headerSize = CollectionUtil.isNotEmpty(headers) ? headers.size()
				: 0;
		boolean hasTitle = CollectionUtil.isNotEmpty(beanInfo.getTitles());
		Object[] beans = beanInfo.getBeans();
		List<String> datas = CollectionUtil.createArrayList(headerSize
				+ ((hasTitle) ? 1 : 0) + beans.length);
	...
	private Field[] getField(final FileBeanInfo beanInfo) {
		...
		return fieldCache.get(object.getClass().getName(),
				new Callable<Field[]>() {
					@Override
					public Field[] call() throws Exception {
						List<Field> fields = CollectionUtil
								.createArrayList(beanInfo.getFields().size());
						// FIXME
						for (String field : beanInfo.getFields()) {
							fields.add(ReflectionUtil.getField(object,
									field));
						}
	...
	@Override
	public void importBeans(FileBeanInfo beanInfo, String filePath)
			throws IOException {
