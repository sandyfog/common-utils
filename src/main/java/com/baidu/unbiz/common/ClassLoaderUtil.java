 ...
 * <ul>
 * <li>
 * 工具类<code>A</code>是从系统<code>ClassLoader</code>装入的(classpath)</li>
 * <li>
 * 类<code>B</code>是Web Application中的一个类，是由servlet引擎的<code>ClassLoader</code>
 * 动态装入的</li>
 * <li>
 * 资源文件<code>C.properties</code>也在Web Application中，只有servlet引擎的动态
 * <code>ClassLoader</code>可以找到它</li>
 * <li>
 * 类<code>B</code>调用工具类<code>A</code>的方法，希望通过类<code>A</code>取得资源文件
 ...
 * @version create on 2014年7月14日 下午3:11:55
 */
public abstract class ClassLoaderUtil {
	 ...
	 *            要装入的类名
	 * @param referrer
	 *            调用者类，如果为<code>null</code>，则该方法相当于<code>Class.forName</code>
	 *
	 * @return 已装入的类
	 *
	 * @throws ClassNotFoundException
	 *             如果类没找到
	 */
	public static <T> Class<T> loadClass(String className, Class<?> referrer)
			throws ClassNotFoundException {
	 ...
	 *            要装入的类名
	 * @param classLoader
	 *            从指定的<code>ClassLoader</code>中装入类，如果为<code>null</code>，表示从
	 *            <code>ClassLoaderUtil</code>所在的class loader中装载
	 *
	 * @return 已装入的类
	 *
	 * @throws ClassNotFoundException
	 ...
	 *
	 * @throws ClassNotFoundException
	 *             如果service class找不到，或装载service文件失败
	 */
	public static <T> Class<T> loadServiceClass(String serviceId)
			throws ClassNotFoundException {
	 ...
	 *            服务名
	 * @param referrer
	 *            调用者类，如果为<code>null</code>，表示在<code>ClassLoaderUtil</code>
	 *            的class loader中找。
	 *
	 * @return service class
	 *
	 ...
	 *            服务名
	 * @param classLoader
	 *            在指定classLoader中查找，如果为<code>null</code>，表示在
	 ...
	 *             如果service class找不到，或装载service文件失败
	 */
	public static <T> Class<T> loadServiceClass(String className,
			String serviceId) throws ClassNotFoundException {
	 ...
	 */
	public static <T> Class<T> loadServiceClass(String className,
			String serviceId, Class<T> referrer) throws ClassNotFoundException {
		ClassLoader classLoader = getReferrerClassLoader(referrer);
	 ...
	 */
	public static <T> Class<T> loadServiceClass(String className,
			String serviceId, ClassLoader classLoader)
			throws ClassNotFoundException {
		try {
			// 先尝试装入class。
	 ...
	 *            要实例化的类名
	 * @param referrer
	 *            调用者类，如果为<code>null</code>，则从<code>ClassLoaderUtil</code>
	 *            所在的class loader装载
	 *
	 * @return 指定类名的实例
	 *
	...
	public static <T> T newInstance(Class<T> clazz)
		...
		try {
			...
			throw new ClassInstantiationException(clazz,
					"Failed to instantiate class: " + clazz.getName(), e);
	 ...
	 *             如果实例化失败
	 */
	public static <T> T newServiceInstance(String serviceId)
			throws ClassNotFoundException, ClassInstantiationException {
	 ...
	 */
	public static <T> T newServiceInstance(String className, String serviceId,
			Class<T> referrer) throws ClassNotFoundException,
			ClassInstantiationException {
	 ...
	 *            要查找的资源名，就是以&quot;/&quot;分隔的标识符字符串
	 * @param classLoader
	 *            从指定的<code>ClassLoader</code>中查找
	 *
	 * @return resource的URL数组，如果没找到，则返回空数组。数组中保证不包含重复的URL。
	 */
	public static URL[] getResources(String resourceName,
	 ...
	 * @param classLoader
	 *            类装入器
	 * @param sysClassLoader
	 *            是否用system class loader装载资源
	 *
	 * @return 如果找到，则返回<code>true</code>
	 */
	private static boolean getResources(List<URL> urlSet, String resourceName,
		...
		if (CollectionUtil.hasNotItems(enums)) {
			return false;
		}
