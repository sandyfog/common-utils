...
public abstract class CsvUtil {
	...
	public static String toCsvString(Object... elements) {
		...
		for (int i = 0, last = elements.length - 1; i <= last; i++) {
			...
			if (ndx == -1
					&& (field.startsWith(StringPool.Symbol.SPACE) || field
							.endsWith(StringPool.Symbol.SPACE))) {
				ndx = 1;
			}
			ndx = (ndx == -1) ? StringUtil.indexOfChars(field, SPECIAL_CHARS)
					: ndx;
			...
			line.append(StringUtil.replace(field,
					StringPool.Symbol.DOUBLE_QUOTE, DOUBLE_QUOTE));
			if (ndx != -1) {
				line.append(FIELD_QUOTE);
			}
	...
	private static void addField(List<String> row, String line, int startIndex,
			int endIndex, boolean inQuoted) {
