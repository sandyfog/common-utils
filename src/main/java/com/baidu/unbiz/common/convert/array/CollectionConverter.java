...
public class CollectionConverter<T> implements TypeConverter<Collection<T>> {
	...
	public CollectionConverter(Class<? extends Collection<?>> collectionType,
		...
		this(ConverterManagerBean.getInstance(), collectionType,
				targetComponentType);
	}
	...
	protected Collection<T> createCollection(int length) {
		if (collectionType.isInterface()) {
			...
			throw new ConvertException("Unknown collection: "
					+ collectionType.getName());
		}
		if (length > 0) {
			try {
				@SuppressWarnings("unchecked")
				Constructor<Collection<T>> constructor = (Constructor<Collection<T>>) collectionType
		...
		try {
			@SuppressWarnings("unchecked")
			Collection<T> collection = (Collection<T>) collectionType
					.newInstance();
			return collection;
		} catch (Exception e) {
