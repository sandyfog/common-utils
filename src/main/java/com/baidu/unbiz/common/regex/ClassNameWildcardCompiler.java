 ...
 * @version create on 2014年9月19日 上午2:34:56
 */
public class ClassNameWildcardCompiler {
	...
	private static final String REGEX_CLASS_NAME_SINGLE_CHAR = "("
			+ REGEX_CLASS_NAME_CHAR + ")";
	private static final String REGEX_CLASS_NAME = "(" + REGEX_CLASS_NAME_CHAR
			+ "*)";
	private static final String REGEX_CLASS_NAME_FULL = "("
			+ REGEX_CLASS_NAME_CHAR + "+(?:" + REGEX_DOT_NO_DUP
			+ REGEX_CLASS_NAME_CHAR + "*)*(?=" + REGEX_DOT + "|$)|)"
			+ REGEX_DOT + "?";
	...
	public static String classNameToRegex(String pattern, int options)
			throws PatternSyntaxException {
		pattern = Assert.assertNotNull(normalizeClassName(pattern), "pattern");
		...
		for (int i = 0; i < pattern.length(); i++) {
			...
			case DOT:
				...
				if (lastToken == LAST_TOKEN_DOT
						|| lastToken == LAST_TOKEN_START) {
			...
			case QUESTION:
				if (lastToken == LAST_TOKEN_START) {
					buf.append(REGEX_WORD_BOUNDARY).append(
							REGEX_CLASS_NAME_SINGLE_CHAR); // 前边界
				} else if (i + 1 == pattern.length()) {
					buf.append(REGEX_CLASS_NAME_SINGLE_CHAR).append(
							REGEX_END_OF_NAME); // 后边界
				} else {
	...
	public static String normalizeClassName(String name) {
		...
		name = name.replaceAll("^\\.|\\.$", Emptys.EMPTY_STRING);
