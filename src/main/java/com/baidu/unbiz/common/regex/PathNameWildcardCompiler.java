 ...
 * @version create on 2014年9月19日 上午2:37:37
 */
public class PathNameWildcardCompiler {
	...
	private static final String REGEX_FILE_NAME_SINGLE_CHAR = "("
			+ REGEX_FILE_NAME_CHAR + ")";
	...
	private static final String REGEX_FILE_PATH = "(" + REGEX_FILE_NAME_CHAR
			+ "+(?:" + REGEX_SLASH_NO_DUP + REGEX_FILE_NAME_CHAR + "*)*(?="
			+ REGEX_SLASH + "|$)|)" + REGEX_SLASH + "?";
	private static final String REGEX_END_OF_PATH = "(?=" + REGEX_SLASH + "|$)"
			+ REGEX_SLASH + "?";
	...
	public static String pathNameToRegex(String pattern, int options)
			throws PatternSyntaxException {
		pattern = Assert.assertNotNull(normalizePathName(pattern), "pattern");
		...
		if (forceMatchPrefix || pattern.length() > 0
		...
		for (int i = 0; i < pattern.length(); i++) {
			...
			if (forceAbsolutePath && lastToken == LAST_TOKEN_START
					&& ch != SLASH) {
			...
			case STAR:
				...
				if (j < pattern.length() && pattern.charAt(j) == STAR) {
					...
					if (lastToken != LAST_TOKEN_START
							&& lastToken != LAST_TOKEN_SLASH) {
						throw new PatternSyntaxException("Syntax Error",
								pattern, i);
					}
			...
			case QUESTION:
				if (lastToken == LAST_TOKEN_START) {
					buf.append(REGEX_WORD_BOUNDARY).append(
							REGEX_FILE_NAME_SINGLE_CHAR); // 前边界
				} else if (i + 1 == pattern.length()) {
					buf.append(REGEX_FILE_NAME_SINGLE_CHAR).append(
							REGEX_END_OF_PATH); // 后边界
				} else {
