...
/**
 * <code>JWhich</code>是一个类似UNIX的<code>which</code>
 * 命令的小工具，所不同的是，它可以查找指定的java类或资源是从什么地方装载的。
 * 
 * <p>
 ...
 */
public class JWhich {
	private static final String[] CLASSPATH = SystemUtil.getJavaRuntimeInfo()
			.getClassPathArray();

	/**
	 * 在当前线程的context class loader下查找指定的类或资源，并全部列出来。
	 * 
	 * @param classOrResourceName
	 *            类名或资源名
	 */
	public static void which(String classOrResourceName) {
		...
		if ((classURLs.length == 0) && (resourceURLs.length == 0)) {
			System.out.println("\nClass or resource '" + classOrResourceName
					+ "' not found.");
		} else {
			if (classURLs.length > 0) {
				System.out.println("\nClass '" + classOrResourceName
						+ "' found in: \n");

				for (int i = 0; i < classURLs.length; i++) {
					System.out.println(StringUtil.alignRight("(" + (i + 1)
							+ ")", 6)
							+ " " + classURLs[i]);
				}
			}
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			System.out.println("\nUsage:");
			System.out.println("    java " + JWhich.class.getName()
					+ " MyClass");
			System.out.println("    java " + JWhich.class.getName()
					+ " my.package.MyClass");
			System.out.println("    java " + JWhich.class.getName()
