...
public class Occupy {
	...
	public Occupy(byte nullReferenceSize, byte emptyObjectSize,
			byte emptyArrayVarSize, String name) {
		this.NULL_REFERENCE_SIZE = nullReferenceSize;
		this.EMPTY_OBJECT_SIZE = emptyObjectSize;
		this.EMPTY_ARRAY_VAR_SIZE = emptyArrayVarSize;
		this.name = name;
	...
	public static Occupy forDetectedVM() {
		...
		if (sunVendorURL.equals(vendorURL)) {
			String sunArchDataModel = JVMUtil.getJavaRuntimeInfo()
					.getSunArchDataModel();
	...
	private Map<Object, Object> identityMap = CollectionUtil
	...
	private int sizeofArray(Class<?> clazz, Object object, int initSize) {
		...
		if (componentType.isPrimitive()) {
			return initSize + lengthOfPrimitiveArray(object)
					* sizeofPrimitiveClass(componentType);
		}
