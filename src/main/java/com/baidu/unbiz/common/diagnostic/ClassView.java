...
public class ClassView {
	...
	private static String toInterfaceString(Class<?> clazz, String indent) {
		...
		builder.append(indent).append("Interface ").append(clazz.getName())
				.append("  (").append(toClassString(clazz)).append(')');
	...
	private static String toClassString(Class<?> clazz, String indent) {
		...
		builder.append(indent).append("Class ").append(clazz.getName())
				.append("  (").append(toClassString(clazz)).append(')');
		...
		for (int i = 0, c = interfaceClass.length; i < c; ++i) {
			builder.append('\n').append(
					toInterfaceString(interfaceClass[i], indent));
		}
	...
	private static String toClassString(Class<?> clz) {
		...
		return "loaded by "
				+ ObjectUtil.identityToString(loader, "System ClassLoader")
	...
	public static void main(String[] args) throws ClassNotFoundException {
		if (args.length == 0) {
			System.out.println("\nUsage:");
			System.out.println("    java " + ClassView.class.getName()
					+ " MyClass");
			System.out.println("    java " + ClassView.class.getName()
					+ " my.package.MyClass");
			System.out.println("    java " + ClassView.class.getName()
