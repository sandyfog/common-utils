...
import com.baidu.unbiz.common.internal.Entities;
...
public abstract class StringEscapeUtil {
	 ...
	 *             如果输出失败
	 */
	public static void unescapeHtml(String str, Appendable out)
			throws IOException {
	 ...
	 *            要编码的字符串，可以是<code>null</code>
	 * @param encoding
	 *            输出字符编码，如果为<code>null</code>，则使用当前线程默认的编码
	 * @param strict
	 *            是否以严格的方式编码URL
	 * @return URL编码后的字符串
	 * @throws UnsupportedEncodingException
	 ...
	 */
	public static void escapeURL(String str, String encoding, Appendable out)
			throws IOException {
	 ...
	 *             <code>out</code>为<code>null</code>
	 */
	public static void unescapeURL(String str, String encoding, Appendable out)
			throws IOException {
	 ...
	 *             <code>out</code>为<code>null</code>
	 */
	private static boolean unescapeURLInternal(String str, String encoding,
			Appendable out) throws IOException {
		if (encoding == null) {
			encoding = LocaleUtil.getContext().getCharset().name();
		}
		...
		for (int i = 0; i < length; i++) {
			...
			if (pos > 0) {
				...
				if (!needToChange
						&& !s.equals(new String(charArray, startIndex, pos))) {
					needToChange = true;
				}
