...
class DefaultIpManager extends CachedLogger implements IpManager, IpLib {
	...
	private List<String> loadRowData() {
		...
		try {
			return ReaderUtil.readLinesAndClose(this.getClass()
					.getResourceAsStream(ipData.getDataFile()));
		} catch (IOException e) {
	...
	private synchronized void loadIpLib() {
		...
		logger.infoIfEnabled(
				"load IP lib finished. have {}records, spend {}ms.", total,
				(end - start));
		init = true;
	}
	...
	private void transformData(List<String> lines) {
		...
		for (Iterator<String> iter = lines.iterator(); iter.hasNext(); total++) {
			...
			ipInfoArray[total] = StringUtil.substringAfter(line,
					ipData.getSplit()).trim();
