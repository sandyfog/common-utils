...
public abstract class ClassUtil {
	...
	private static final Map<String, PrimitiveInfo<?>> PRIMITIVES = CollectionUtil
			.createHashMap();
	...
	/**
	 * 判断是否为原子类型或者其包装类，如果<code>clazz</code>为<code>null</code>，则返还
	 * <code>false</code>
	 */
	...
	/**
	 * 如果对象<code>object</code>是类<code>clazz</code>的实例， 则返回<code>true</code>，否则返回
	 * <code>false</code>
	 * <p>
	 * 此方法考虑包装类和基本类型的转换
	 * 
	 ...
	 * @param object
	 *            校验的对象
	 * @return 是否匹配，如果<code>clazz</code>或者<code>object</code>为<code>null</code>
	 *         ，则返回<code>false</code>
	 */
	 ...
	 * 按一下顺序判断：
	 * <li>
	 * 如果<code>clazz</code>为<code>primitive</code>类型，我们认为除了
	 * <code>void.class</code>外，其他都可实例化，因为可以直接赋值</li>
	 * <li>
	 * 如果<code>clazz</code>为<code>Array</code>类型，如果数组元素的类型为<code>public</code>
	 * ，那么显然也可以实例化，因为可以直接定义新对象，返还 <code>true</code></li>
	 * <li>接下来看<code>clazz</code>是否为<code>abstract</code>或
	 * <code>interface ，如果是，则显然不可实例化，返还<code>false</code></li>
	 * <li>最后遍历<code>clazz</code>的构造器，查看是否存在<code>public</code>类型接口，如果存在则返还
	 * <code>true</code>，否则返还<code>false</code></li>
	 * 
	 * @param clazz
	 *            校验的类
	...
	public static <T> boolean canInstatnce(Class<T> clazz) {
		...
		if (clazz.isArray()
				&& Modifier.isPublic(getArrayComponentType(clazz)
						.getModifiers())) {
			return true;
		}
	...
	protected static class TypeInfo {
		...
		private TypeInfo(Class<?> type) {
			...
			if (dimension == 0) {
				...
				for (Iterator<Class<?>> i = superclasses.iterator(); i
						.hasNext();) {
				...
				for (Iterator<Class<?>> i = getTypeInfo(componentType).interfaces
						.iterator(); i.hasNext();) {
	...
	private final static Map<Class<?>, Set<Class<?>>> assignmentTable = CollectionUtil
			.createHashMap();
	...
	static {
		...
		assignmentTable.put(int.class,
				assignableSet(int.class, byte.class, short.class, char.class));

		// long可以接受：long、int、byte、short、char
		assignmentTable.put(
				long.class,
				assignableSet(long.class, int.class, byte.class, short.class,
						char.class));

		// float可以接受：float, long, int, byte, short, char
		assignmentTable.put(
				float.class,
				assignableSet(float.class, long.class, int.class, byte.class,
						short.class, char.class));

		// double可以接受：double, float, long, int, byte, short, char
		assignmentTable.put(
				double.class,
				assignableSet(double.class, float.class, long.class, int.class,
	...
	public static String locateClass(String className, ClassLoader loader) {
		className = Assert.assertNotNull(StringUtil.trimToNull(className),
				"className");
		...
		if (locationURL != null) {
			...
			if (location.endsWith(classFile)) {
				location = location.substring(0,
						location.length() - classFile.length());
			}
	 ...
	 */
	static String toFriendlyClassName(String javaClassName,
	...
	public static String getSimpleClassName(String javaClassName,
		...
		if (proccesInnerClass) {
			...
			return friendlyClassName.substring(friendlyClassName
					.lastIndexOf(".") + 1);
		}
	}
	...
	public static String getSimpleMethodSignature(Method method,
		...
		return getSimpleMethodSignature(method, false, false, withClassName,
				false);
	}
