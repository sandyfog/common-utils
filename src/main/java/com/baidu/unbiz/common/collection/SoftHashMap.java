 ...
 * entry in a <tt>WeakHashMap</tt> will automatically be removed when its key is
 ...
 * key has been discarded its entry is effectively removed from the map, so this
 ...
 * operator. Once such a key is discarded it can never be recreated, so it is
 * impossible to do a lookup of that key in a <tt>WeakHashMap</tt> at some later
 * time and be surprised that its entry has been removed. This class will work
 * perfectly well with key objects whose <tt>equals</tt> methods are not based
 * upon object identity, such as <tt>String</tt> instances. With such
 ...
 * <tt>Map</tt> invariants do not hold for this class. Because the garbage
 * collector may discard keys at any time, a <tt>WeakHashMap</tt> may behave as
 * though an unknown thread is silently removing entries. In particular, even if
 * you synchronize on a <tt>WeakHashMap</tt> instance and invoke none of its
 * mutator methods, it is possible for the <tt>size</tt> method to return
 * smaller values over time, for the <tt>isEmpty</tt> method to return
 * <tt>false</tt> and then <tt>true</tt>, for the <tt>containsKey</tt> method to
 * return <tt>true</tt> and later <tt>false</tt> for a given key, for the
 ...
 * be taken to ensure that value objects do not strongly refer to their own
 * keys, either directly or indirectly, since that will prevent the keys from
 * being discarded. Note that a value object may refer indirectly to its key via
 * the <tt>WeakHashMap</tt> itself; that is, a value object may strongly refer
 * to some other key object whose associated value object, in turn, strongly
 * refers to the key of the first value object. One way to deal with this is to
 * wrap values themselves within <tt>SoftReferences</tt> before inserting, as
 * in: <tt>m.put(key, new SoftReference(value))</tt>, and then unwrapping upon
 * each <tt>get</tt>.
 * 
 * <p>
