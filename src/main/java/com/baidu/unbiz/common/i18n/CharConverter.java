...
public abstract class CharConverter {
	...
	// 私有变量
	private static final Map<String, CharConverter> converters = CollectionUtil
			.createConcurrentMap();
	...
	public static final CharConverter getInstance(String name) {
		...
		if (converter == null) {
			...
			try {
				provider = (CharConverterProvider) ClassLoaderUtil
						.newServiceInstance("char.converter." + name);
			} catch (ClassInstantiationException e) {
				throw new IllegalArgumentException(
						"Failed to load char converter provider: " + name
								+ ": " + e.getMessage());
