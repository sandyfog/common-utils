 ...
 * @version create on 2014年7月21日 上午8:00:37
 */
public class LocaleUtil {
	...
	public static boolean isLocaleSupported(Locale locale) {
		return locale != null
				&& AvailableLocalesLoader.locales.AVAILABLE_LANGUAGES
						.contains(locale.getLanguage())
				&& AvailableLocalesLoader.locales.AVAILABLE_COUNTRIES
						.contains(locale.getCountry());
	}
	...
	/**
	 * 取得备选的resource bundle风格的名称列表。 FIXME，此方法恶心
	 * <p>
	 * 例如：
	...
	private static Notifier[] getNotifiers() {
		try {
			URL[] files = ClassLoaderUtil.getResources(
					"META-INF/services/localeNotifiers", ClassLoaderUtil.class);
			List<Notifier> list = CollectionUtil.createLinkedList();

			for (URL file : files) {
				for (String className : StringUtil.split(
						StreamUtil.readText(file.openStream(), "UTF-8", true),
						"\r\n ")) {
					list.add(Notifier.class.cast(ClassLoaderUtil.newInstance(
							className, ClassLoaderUtil.class)));
				}
			}
	...
	private static class AvailableLocales {
		private final Set<String> AVAILABLE_LANGUAGES = CollectionUtil
				.createHashSet();
		private final Set<String> AVAILABLE_COUNTRIES = CollectionUtil
				.createHashSet();
