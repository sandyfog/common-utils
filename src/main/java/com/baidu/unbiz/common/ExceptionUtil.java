...
public abstract class ExceptionUtil {
	...
	public static boolean causedBy(Throwable throwable,
		...
		for (; throwable != null && !causeType.isInstance(throwable)
				&& !causes.contains(throwable); throwable = throwable
				.getCause()) {
	...
	public static List<Throwable> getCauses(Throwable throwable,
		...
		for (; throwable != null && !causes.contains(throwable); throwable = throwable
				.getCause()) {
			if (reversed) {
	...
	/**
	 * 抛出Throwable，但不需要声明<code>throws Throwable</code>，区分<code>Exception</code>
	 * 或</code>Error</code>。
	 * 
	 * @param throwable
	 *            受检异常
	 * @throws Exception
	 */
	public static void throwExceptionOrError(Throwable throwable)
			throws Exception {
	 ...
	 * <code>RuntimeException</code>、<code>Exception</code>
	 * 或</code>Error</code>。
	 * 
	 * @param throwable
	 *            受检异常
