...
public class MethodCache {
	...
	private final Computable<String, Map<String, Method>> cachedMap = new ConcurrentCache<String, Map<String, Method>>();
	...
	public Method[] getMethods(final Class<?> clazz) {
		...
		return cache.get(ClassUtil.getFriendlyClassName(clazz),
				new Callable<Method[]>() {
					@Override
	...
	public Method[] getInstanceMethods(final Class<?> clazz) {
		...
		return cache.get(ClassUtil.getFriendlyClassName(clazz) + ".instance",
				new Callable<Method[]>() {
					@Override
					public Method[] call() throws Exception {
						return ReflectionUtil
								.getAllInstanceMethods(clazz, true);
					}
				});
	}

	/**
	 * 生成并缓存<code>clazz</code>的所有带<code>annotationClass</code>的
	 * <code>Field</code>
	 * 
	 * @param clazz
	 *            目标class
	...
	public Method[] getMethods(final Class<?> clazz,
		...
		return cache.get(ClassUtil.getFriendlyClassName(clazz) + "."
				+ annotationClass.getName(), new Callable<Method[]>() {
			@Override
			public Method[] call() throws Exception {
				return ReflectionUtil.getAnnotationMethods(clazz,
						annotationClass).toArray(new Method[0]);
			}
		});
	}
	...
	private static String getMethodSignature(String methodName,
		...
		String name = methodName
				+ (ArrayUtil.isEmpty(params) ? "" : ArrayUtil.toString(params));

		return name;
	}

	public Method getMethod(final Class<?> clazz, String methodName,
			Class<?>... params) {
		if (clazz == null) {
			return null;
		}

		Map<String, Method> map = cachedMap.get(
				ClassUtil.getFriendlyClassName(clazz),
				new Callable<Map<String, Method>>() {
					@Override
					public Map<String, Method> call() throws Exception {
						Method[] methods = getMethods(clazz);
						Map<String, Method> methodMap = CollectionUtil
								.createHashMap(methods.length);

						for (Method method : methods) {
							Class<?>[] types = method.getParameterTypes();
							String name = getMethodSignature(method.getName(),
									types);
	...
	public Method getInstanceMethod(final Class<?> clazz, String methodName,
			Class<?>... params) {
		if (clazz == null) {
			return null;
		}
		...
		Map<String, Method> map = cachedMap.get(
				ClassUtil.getFriendlyClassName(clazz) + ".instance",
				new Callable<Map<String, Method>>() {
					@Override
					public Map<String, Method> call() throws Exception {
