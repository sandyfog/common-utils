...
import com.baidu.unbiz.common.bean.BeanUtil;

/**
 * 
 * @author <a href="mailto:xuchen06@baidu.com">xuc</a>
 * @version create on 2014年9月19日 上午1:10:13
 */
public class Properties {
	...
	protected Map<String, PropertyDescriptor> inspectProperties() {
		...
		Map<String, PropertyDescriptor> map = CollectionUtil.createHashMap();

		Method[] methods = scanAccessible ? ReflectionUtil
				.getAccessibleMethods(type) : ReflectionUtil
				.getAllMethodsOfClass(type);
			...
			for (Method method : methods) {
				...
				if (add == true) {
					MethodDescriptor methodDescriptor = classDescriptor
							.getMethodDescriptor(method.getName(),
		...
		if (classDescriptor.isIncludeFieldsAsProperties()) {
			FieldDescriptor[] fieldDescriptors = classDescriptor
			...
			for (FieldDescriptor fieldDescriptor : fieldDescriptors) {
				...
				if (!map.containsKey(name)) {
					...
					map.put(name,
							createPropertyDescriptor(name, fieldDescriptor));
				}
			}
	...
	protected void addProperty(Map<String, PropertyDescriptor> map,
			String name, MethodDescriptor methodDescriptor, boolean isSetter) {
		...
		if (existing == null) {
			...
			PropertyDescriptor propertyDescriptor = createPropertyDescriptor(
					name, getterMethod, setterMethod);
		...
		if (!isSetter) {
			...
			// check existing
			MethodDescriptor existingMethodDescriptor = existing
			...
			if (getterMethod.getMethod().getReturnType() != setterMethod
					.getMethod().getParameterTypes()[0]) {
				return;
			}
		}

		PropertyDescriptor propertyDescriptor = createPropertyDescriptor(name,
	...
	protected PropertyDescriptor createPropertyDescriptor(String name,
			MethodDescriptor getterMethod, MethodDescriptor setterMethod) {
		return new PropertyDescriptor(classDescriptor, name, getterMethod,
				setterMethod);
	}
	...
	public PropertyDescriptor[] getAllPropertyDescriptors() {
		if (allProperties == null) {
			PropertyDescriptor[] allProperties = new PropertyDescriptor[propertyDescriptors
					.size()];
