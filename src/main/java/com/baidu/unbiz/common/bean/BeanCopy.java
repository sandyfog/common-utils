...
public abstract class BeanCopy {
	...
	private static final BeanInfoCache beanInfoCache = BeanInfoCache
			.getInstance();
	...
	public static void copyProperties(Object src, Object dest) {
		...
		for (Field field : destFields) {
			...
			Field srcField = fieldCache.getInstanceField(srcClazz,
					field.getName());
			if (srcField != null) {
	...
	public static void copyByAnnotation(Object src, Object dest) {
		...
		for (Field field : destFields) {
			if (ReflectionUtil.isFinal(field)
					|| ReflectionUtil.hasAnnotation(field, IgnoreField.class)) {
				continue;
			}
			Field srcField = fieldCache.getField(srcClazz, CopyField.class,
					field.getName());
			if (srcField != null
					&& supportFor(destClazz,
							srcField.getAnnotation(CopyField.class))) {
				Object value = ReflectionUtil.readField(srcField, src);
				ReflectionUtil.writeField(field, dest, value);
			}
	...
	public static void copyByMethod(Object src, Object dest) {
		...
		for (PropertyDescriptor destPd : propertyDescriptors) {
			PropertyDescriptor srcPd = beanInfoCache.getPropertyDescriptor(
					src.getClass(), destPd.getName());
			if (srcPd == null) {
				continue;
			}
