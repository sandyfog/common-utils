...
public class Methods {
	...
	protected Map<String, MethodDescriptor[]> inspectMethods() {
		...
		Map<String, MethodDescriptor[]> map = CollectionUtil
	...
	public MethodDescriptor getMethodDescriptor(String name) {
		...
		if (methodDescriptors.length != 1) {
			throw new IllegalArgumentException("Method name not unique: "
					+ name);
		}
	...
	public MethodDescriptor[] getAllMethodDescriptors() {
		if (allMethods == null) {
			List<MethodDescriptor> allMethodsList = CollectionUtil
					.createArrayList();
			...
			MethodDescriptor[] allMethods = allMethodsList
					.toArray(new MethodDescriptor[allMethodsList.size()]);

			Arrays.sort(allMethods, new Comparator<MethodDescriptor>() {
				public int compare(MethodDescriptor md1, MethodDescriptor md2) {
					return md1.getMethod().getName()
